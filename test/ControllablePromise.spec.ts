...
describe('ControllablePromise', () => {
  ...
  describe('progress', () => {
    it('emit progress to multiple listeners', (done) => {
      ...
      const cp = new ControllablePromise((resolve, reject, progress) => {
        setTimeout(() => {
          progress(expectedStats = { percent: 10, files: { count: 1, total: 10 } })
          progress(expectedStats = { percent: 30, files: { count: 3, total: 10 } })
          progress(expectedStats = { percent: 50, files: { count: 5, total: 10 } })
          progress(expectedStats = { percent: 100, files: { count: 10, total: 10 } })
          setTimeout(resolve)
        })
      })
    ...
    it('onProgress should return this', (done) => {
      ...
      const shouldBeCp = cp.onProgress((stats) => assert.deepStrictEqual(stats, {}))
      shouldBeCp
        .then(() => {
  ...
  describe('cancel', () => {
      ...
      setTimeout(() => {
        ...
        cancelPromise
          ...
          .catch((error) => {
            ...
            assert.strictEqual(error.name, 'ControllablePromisePreconditionError')
    ...
    it('cancel should fail if the promise has settled during cancel handler', (done) => {
      ...
      cp.then(() => hasResolve = true)
        .catch(done)

      cp.cancel().catch((error) => {
        assert(hasResolve)
        assert.strictEqual(cancelHandlerRunCount, 1)
        assert.strictEqual(error.message, 'Promise was settled during cancel handler')
        done()
      }).catch(done)
    })
    ...
    it('if no onCancel handler, cancel when the promise is finished', (done) => {
      ...
      cp.then(() => done(new Error('Should fail')))
        .catch((error) => {
          assert.strictEqual(error.name, 'ControllablePromiseCancelError')
          assert.strictEqual(error.message, 'Controllable Promise was canceled')
          done()
        })
        .catch(done)
    ...
    it('if no onCancel handler, cancel should fail if error occur in main promise', (done) => {
      ...
      cp.then(() => done(new Error('Should fail')))
        .catch(() => assert(cp.isRejected))

      const cancelPromise = cp.cancel()
      // assert(cp.lock)
      cancelPromise.then(() => done(new Error('Should fail')))
        .catch((error) => {
          assert(cp.isRejected)
          // assert(!cp.lock)
          assert.strictEqual(error.message, expectedMessage)
  ...
  describe('pause', () => {
    ...
    it('pause should fail if the promise is already settled', (done) => {
      ...
      setTimeout(() => {
        const pausePromise = cp.pause()
        // assert(!cp.lock)
        pausePromise.then(() => done(new Error('Should fail')))
          .catch((error) => {
            assert(!cp.isResumed)
            assert(!cp.isPaused)
            assert(!cp.isCanceled)
    ...
    it('if no onPause handler, pause when the promise is finished', (done) => {
      ...
      cp.then(() => done(new Error('Should not resolve or reject')))
        .catch(done)
