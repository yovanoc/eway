...
export class ControllablePromise<T> implements IControllable<T> {
  ...
  get isSettled(): boolean {
    return this.state === ControllablePromiseState.CANCELED || this.state === ControllablePromiseState.REJECTED || this.state === ControllablePromiseState.FULFILLED;
  }
  ...
  constructor(executor: ControllablePromiseExecutor<T>) {
    ...
    this.promise = new Promise<T>((resolveMain, rejectMain) => {
      ...
      const pause = (pauseFn: PromiseExecutor<void>) => this.pauseFn = pauseFn
      const resume = (resumeFn: PromiseExecutor<void>) => this.resumeFn = resumeFn
      const cancel = (cancelFn: PromiseExecutor<void>) => this.cancelFn = cancelFn
  ...
  public pause() {
    return new Promise<void>((resolvePause, rejectPause) => {
      if (this.lock) {
        return rejectPause(new ControllablePromisePreconditionError("Operation in progress"));
      }
      if (this.isCanceled) {
        return rejectPause(new ControllablePromisePreconditionError("Promise was canceled"));
      }
      if (this.isSettled) {
        return rejectPause(new ControllablePromisePreconditionError("Promise was settled"));
      }
      if (this.isPaused) {
        return resolvePause();
      }
  ...
  public cancel() {
    return new Promise<void>((resolveCancel, rejectCancel) => {
      if (this.lock) {
        return rejectCancel(new ControllablePromisePreconditionError("Operation in progress"));
      }
      if (this.isCanceled) {
        return rejectCancel(new ControllablePromisePreconditionError("Promise was canceled"));
      }
      if (this.isSettled) {
        return rejectCancel(new ControllablePromisePreconditionError("Promise was settled"));
      }
      if (this.isCanceled) {
